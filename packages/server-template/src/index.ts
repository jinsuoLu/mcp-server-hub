 ...
 *   4. Add to cli registry (packages/cli/src/index.ts â†?REGISTRY array)
...
server.setRequestHandler(CallToolRequestSchema,async(request)=>{try{const{name,arguments:args}=request.params;switch(name){case"hello":{const n=requireParam(args,"name");return success(`ðŸ‘‹ Hello ${n}! This is the template server. Replace me with your own tools!`)}
case"ping":return success("ðŸ“ Pong! Template server is running.");default:return error(`Unknown tool:${name}`)}}catch(e){return error(e.message)}});
async function main(){const transport=new StdioServerTransport();await server.connect(transport);console.error("ðŸ·ï¸?MCP Template Server running on stdio")}
